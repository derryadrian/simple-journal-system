defmodule SimpleJournalSystemWeb.UserAuthTest do
  ...
  import SimpleJournalSystem.AccountsFixtures

  @remember_me_cookie "_simple_journal_system_web_user_remember_me"
  @remember_me_cookie_max_age 60 * 60 * 24 * 14

  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, SimpleJournalSystemWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "log_in_user/3" do
    ...
    test "writes a cookie if remember_me was set in previous session", %{conn: conn, user: user} do
      ...
      conn =
        ...
        |> Map.replace!(
          :secret_key_base,
          SimpleJournalSystemWeb.Endpoint.config(:secret_key_base)
        )
        |> fetch_cookies()
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      SimpleJournalSystemWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        SimpleJournalSystemWeb.Endpoint.subscribe("users_sessions:#{Base.url_encode64(token)}")
      end
