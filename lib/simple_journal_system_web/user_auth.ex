defmodule SimpleJournalSystemWeb.UserAuth do
  ...
  def log_out_user(conn) do
    ...
    if live_socket_id = get_session(conn, :live_socket_id) do
      SimpleJournalSystemWeb.Endpoint.broadcast(live_socket_id, "disconnect", %{})
    end
  ...
  defp renew_session(conn, user) when conn.assigns.current_scope.user.user_id == user.user_id do
    conn
  end
  ...
  def disconnect_sessions(tokens) do
    Enum.each(tokens, fn %{token: token} ->
      SimpleJournalSystemWeb.Endpoint.broadcast(user_session_topic(token), "disconnect", %{})
    end)
  end
